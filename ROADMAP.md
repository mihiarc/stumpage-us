...
- [x] Re-verified the 5 stale, 3 unverified and 2 dead entries. Two were wrong:
      the USFS SRS directory had **moved**, not died (research.fs.usda.gov,
      updated 2025-06-02), and Virginia Tech's URL had changed.
...
- [ ] Attach the `stumpage.us` domain — **blocked, not started.** As of
      2026-08-30 the domain has no DNS records at all. Runbook is in
      `README.md`; the CNAME and `NEXT_PUBLIC_BASE_PATH` must change together
      or the live site breaks.
- [x] Analytics wired (Cloudflare Web Analytics, cookieless) and `/privacy`
      written. Inert until `NEXT_PUBLIC_CF_BEACON_TOKEN` is set, which should
      wait for the domain — measuring the github.io sub-path first would split
      the record across two hostnames.
- [x] SRS-successor framing removed from all seven locations; positioning now
      leads with the record

> **Gate:** the paper can cite a stable URL and a dataset. **The dataset half is
> done and citable today** at `/coverage/coverage.json`. The URL half is still
> the github.io sub-path until DNS is configured.
